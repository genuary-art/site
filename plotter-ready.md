...
* [template C](genuary-22-svg-template-c) ([src](https://github.com/genuary-art/site/blob/main/genuary-22-svg-template-c.html)) might take a while to render. It kinda does the same as template B, except the distortion is based on the lighting of a 3D raymarched scene instead of a noise function, showing the GENUARY logo. It also contains a nice "clip" function, allowing you to clip a bunch of lines against a 2D SDF. It also increases the viewbox resolution to 1e5, and rounds all the coordinates to integers to save more space. It also uses a variation on the path function that uses relative coordinates, and saves even more space in the SVG. And it's also got a line simplification function. It's also pink.
