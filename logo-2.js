...
(code=({copyright:piterpasma,max,floor,abs,sin,cos,min,imul,PI}=Math)=>{
  ...
  gen_img = _=>{
    ...
    L=(x,y)=>(x*x+y*y)**.5; // vec2 length
    L2=(x,y)=>x*x+y*y; // sqr vec2 length
    len3=(x,y,z)=>(x*x+y*y+z*z)**.5; // vec3 length
    H=([x,y],[a,b])=>L(x-a,y-b);
    H2=([x,y],[a,b])=>L2(x-a,y-b);

    A=([x,y,z],[a,b,c],t=1)=>[x+a*t,y+b*t,z+c*t]; // vec3 add
    mix3=([x,y,z],[a,b,c],t=1,s=1-t)=>[x*s+a*t,y*s+b*t,z*s+c*t]; // vec3 mix
    mix2=([x,y],[a,b],t=1,s=1-t)=>[x*s+a*t,y*s+b*t]; // vec2 mix
    mix=(a,b,p)=>a+p*(b-a);
    ...
    fw=fw.map(v=>v*ZM); // pre multiply fwd vector (used later in raytrace fn)
    ...
    GENUARY=(x,y)=>{
      ...
      let G_=k(U(x,y-8),B(k(B(x)-1-x0,ay9-3-x0)-3+x0));//G
      x -= 12;
    ...
    Qh=({x,y,w,p,r},X,Y,W,f)=>X<x+w&&X+W>x&&Y<y+w&&Y+W>y&&(p.some(v=>v[0]>=X&&v[0]<X+W&&v[1]>=Y&&v[1]<Y+W&&f(v))||r&&r.some(s=>Qh(s,X,Y,W,f)));
    Q=(x,y,w)=>({x,y,w,p:[]});
    ...
    function fill_circle(x, y, r, c='#000') {
      // I swear I got this from Stack Overflow or something
      C.beginPath();
      C.fillStyle = c;
      C.moveTo(x + r, y); C.arc(x, y, r, 0, TAU); 
    ...
    function* E() {
      ...
      let tfx=([x,y])=>[(x+Y2)*cw/Y,(y+.5)*ch];
      rek(0,0,cw,ch,'#fc0');
      let NS=9e4,pp=[-2,0],bp=[0,0],bd2=9e9, NO=0,ND=0;
      let LK=33;
      let rv = [];
      let pp0 = R(TAU), pf0 = .1+R(.2), pf3 = .1+R(.2);
      let pp1 = R(TAU), pf1 = R(.1)-.05, pf2 = R(.1)-.05;

      for(let i=0;i<NS;i++){
        let f = i/NS, f1 = 1-f;
        let dx = .01*(T()+T());
        let sx = mix((f)*Y-Y2,R(Y)-Y2,.2);//-Y2*(1-f*2)+dx;
        let sy = R()-.5;
        let x = sx*36, 
            y =-sy*30 + 4 * sin(x*pf2+pp1);
        let b = (.5+.5*sin(y*pf0+x*pf1+pp0))**2*1.7+.3;
        let dd = .5;
        let d = B(GENUARY(x,y)-b)-dd;
        LK--;if(LK<=0){LK=39;yield;}
        if(d<0){
          ND++;
          let DR = 8e-3, MR=DR*4, MR2=MR*MR;
          if(Qh(QT,sx-DR,sy-DR,DR*2,([qu,qv])=>(sx-qu)**2+(sy-qv)**2<DR*DR))continue;
          NO++;
          let bp = [sx,sy];
          Qa(QT,bp);rv.push(bp);
          fill_circle(...tfx(bp),3,'#000');
        }
      }

          // if()continue;
          // let cp = rv[R(NR)|0]=[sx,sy];
          // let d2 = G(H2(pp,cp),MR2);
          // if(d2<bd2){
          //   bp=cp;bd2=d2;
          // }
          // if((i&31)===31){
      ...
      //   let ax=x/ch-Y2,ay=y/ch-.5;
      //   let b=SM(-9,9,-ay*32)+1;
      //   let d=GENUARY(ax*32,-ay*32)-b;
      //   let c=SM(-.5,.5,d)*2-1;
      ...
      console.log(`NO=${NO}=${(100*NO/NS).toFixed(1)}% ND=${(100*ND/NS).toFixed(1)}`);
